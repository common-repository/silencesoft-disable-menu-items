...
class Sil_Disable_Items_Plugin
{
	...
	function sil_disable_update_nav_menu_item($menu_id, $menu_item_id, $args) {
		if ( isset( $_POST[ "sil_disable_menu_item_test_val_$menu_item_id" ] ) && intval ( $_POST[ "sil_disable_menu_item_test_val_$menu_item_id" ] ) == 1 ) {
			update_post_meta( $menu_item_id, 'sil_disable_menu_item_test_val', intval ( $_POST[ "sil_disable_menu_item_test_val_$menu_item_id" ] ) );
		} else {
